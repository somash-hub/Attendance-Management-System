...
continueButton.addEventListener("click", () => {
  ...
  if (!activeStep.querySelector(":invalid")) {
    if (currentStep === 1) {
      ...
      if (!/^\d{8}$/.test(symbol))
        return showMessage("TU Symbol No. must be exactly 8 digits.");
      if (!email.endsWith("@student.tu.edu.np"))
        return showMessage("Use your @student.tu.edu.np college email.");
    }
    if (currentStep === 3) {
      ...
      message.textContent =
        "Account details are valid and ready to connect to your backend.";
